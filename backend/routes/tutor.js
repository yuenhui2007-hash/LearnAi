...
router.post('/ask', authenticate, async (req, res) => {
  ...
  res.json({ answer: `${level} explanation for ${subject || 'question'}`, relatedTopics: ['A', 'B', 'C'], confidence: 0.94 });
});
router.post('/mark', authenticate, async (req, res) => {
  await Session.create({ userId: req.user.id, type: 'marking', question: req.body.content, subject: req.body.subject });
